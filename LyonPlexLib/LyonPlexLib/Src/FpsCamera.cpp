...
void FpsCamera::Update(float deltaTime)
{
	...
	// gestion du mode rotation (clic droit ou alwaysActive)
	if (!m_alwaysActive)
	{
		/*SHORT state = GetAsyncKeyState(VK_RBUTTON);
		bool pressed = (state & 0x8000) != 0;*/

		bool pressed = InputManager::GetKeyIsPressed(VK_RBUTTON);

		if (pressed && !m_rotating)
		{
			m_rotating = true;
			if (m_hWnd && IsWindow(m_hWnd)) SetCapture(m_hWnd);
		...
		{
			m_rotating = false;
			if (m_hWnd && IsWindow(m_hWnd)) ReleaseCapture();
			ClipCursor(nullptr);
			ForceShowCursor();
		}
	}
	...
	// compute quaternion from yaw/pitch (DirectX expects radians)
	float yawR = XMConvertToRadians(m_yaw);
	float pitchR = XMConvertToRadians(m_pitch);
	XMVECTOR q = XMQuaternionRotationRollPitchYaw(pitchR, yawR, 0.0f); // pitch, yaw, roll

	//char buf[256];
	//sprintf_s(buf, "Camera yaw=%.2f pitch=%.2f\n", m_yaw, m_pitch);
	//OutputDebugStringA(buf);

	// write rotation quaternion directly in TransformComponent so other systems read a consistent value
	if (auto* camT = m_camGO->GetComponent<TransformComponent>(); camT) {
		XMFLOAT4 qf;
		XMStoreFloat4(&qf, q);
		camT->rotation = qf; // direct assignation (la struct TransformComponent a bien rotation XMFLOAT4 dans ton code)
		camT->dirty = true;
	}

	// recentre le curseur et met a jour la ref
	CenterCursorInWindow();
}
