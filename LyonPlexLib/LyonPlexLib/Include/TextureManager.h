...
#include "DDSTextureLoader.h"   // DirectXTK helper
...
struct TextureData {
    ...
    {
        ResourceUploadBatch uploadBatch(device->GetDevice().Get());
        uploadBatch.Begin();

        ComPtr<ID3D12Resource> texture;
        HRESULT hr = CreateWICTextureFromFile(device->GetDevice().Get(), uploadBatch, L"texture.png", &texture, true /* mipmaps */);
        ...
        CD3DX12_CPU_DESCRIPTOR_HANDLE hCpuSrv(descMgr->GetSrvHeap()->GetCPUDescriptorHandleForHeapStart(), slotIndex, descriptorSize);
        device->GetDevice()->CreateShaderResourceView(texture.Get(), &srvDesc, hCpuSrv);
        
        uploadBatch.End(device->GetCommandQueue().Get()).wait();
