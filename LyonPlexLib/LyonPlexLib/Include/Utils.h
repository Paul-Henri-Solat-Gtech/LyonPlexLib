...
namespace Utils
{
	...
	// playerAabb    : AABB du joueur (local halfSize) ó appliquez playerScale si besoin avant l'appel
	// obbWorldPos   : centre monde de l'OBB (transform monde)
	// obbWorldOrientation : quaternion monde de l'OBB
	// obbLocal      : OBBCollider local (halfSize, offset) ó appliquez otherScale au halfSize & offset avant l'appel
		...
		float maxSlopeDeg = 45.0f)
	{
		...
		// slab method sur la box centrÈe ‡ l'origine locale
		float tMin = 0.0f;
		...
		for (int i = 0; i < 3; ++i) {
			...
			if (fabsf(d) < 1e-6f) {
				...
				if (t1 > tMin) {
					tMin = t1;
					// normale locale de la face touchÈe sur l'axe i
